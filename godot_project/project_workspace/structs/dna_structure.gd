...
var _include_hydrogens_changed: bool = false
...
func set_edit_mode(in_mode: EditMode) -> void:
	...
	if in_mode == EditMode.SequenceAndPath:
		...
		_springs = {}
		_atom_to_atom_spring_ids = {}
		_atoms_to_related_springs = {}
		locked_atoms = {}
		color_overrides = {}
...
func start_edit() -> void:
	...
	_include_hydrogens_changed = false
	return
	...
	if _edit_mode == EditMode.SequenceAndPath:
		...
		if has_changed:
			...
			if _last_bases_cout != _sequence.length():
				...
				_last_bases_cout = _sequence.length()
			if _last_sequence != _sequence or _include_hydrogens_changed:
				_last_sequence = _sequence
				_atoms_count_cache = -1
				_atoms_ids_cache = {}
				_atoms_cache = {}
...
func set_include_hydrogens(in_include_hydrogens: bool) -> void:
	...
	assert(_edit_mode == EditMode.SequenceAndPath, "Cannot change helix proeprties in this state")
	_signal_queue_parameters_changed = true
	_include_hydrogens_changed = true
...
func _create_hydrogen_bonds() -> void:
	...
	for i: int in _sequence.length():
		...
		var a_template: PackedMolecule = DnaBuilder.get_template(base, get_include_hydrogens())
		var b_template: PackedMolecule = DnaBuilder.get_template(compliment, get_include_hydrogens())
