...
var _has_downloadable_version: bool = false
...
func update_versions_list() -> void:
	...
	if promise.has_error():
		...
		_has_downloadable_version = false
	...
	for version: Dictionary in versions:
		...
		if version.get("is_retracted", false) == false:
			...
			_has_downloadable_version = true
...
func has_downloadable_version() -> bool:
	return _has_downloadable_version
