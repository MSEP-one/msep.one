...
func _on_id_pressed(in_id: int) -> void:
	...
	match in_id:
		...
		ID_DNA_OBJECT:
			...
			request_hide.emit()
		ID_CARBON_NANOTUBE_OBJECT:
			workspace_context.create_object_parameters.set_create_mode_type(CreateObjectParameters.CreateModeType.CREATE_CARBON_NANOTUBE)
