...
signal export_workspace_confirmed(in_workspace: Workspace, in_path: String, in_export_dna_enabled: bool, in_export_nanotube_enabled: bool)
...
func _on_export_file_dialog_file_selected(in_path: String) -> void:
	if !in_path.is_empty():
		...
		assert(workspace.get_ref() != null and workspace.get_ref() is Workspace,
			"Workspace is invalid, cannot save")
		export_workspace_confirmed.emit(
			workspace.get_ref(),
			in_path,
			export_file_dialog.is_export_dna_enabled(),
			export_file_dialog.is_export_nanotube_enabled()
		)
