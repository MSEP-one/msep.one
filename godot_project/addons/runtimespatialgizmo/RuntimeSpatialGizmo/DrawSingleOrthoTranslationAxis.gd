...
@onready var ota_behavior: Node2D = %OTABehavior
...
func update_drawing(in_axis: Node3D, in_side_vec: Vector3, in_front_vec: Vector3, in_axis_color: Color, \
	...
	ota_behavior.camera.global_transform.basis.get_rotation_quaternion() * Vector3.FORWARD
	...
	var camera_to_gizmo_direction: Vector3 = (gizmo.global_position - \
			ota_behavior.camera.global_position).normalized()
	...
	if gizmo.determine_single_axis_in_cutout_perifery(\
			ota_behavior.camera.unproject_position(in_axis.global_position),\
					ota_behavior.camera.unproject_position(gizmo.global_position), \
...
func draw_axis_arrow() -> void:
	...
	var is_orthogonal: bool = ota_behavior.camera.projection == Camera3D.PROJECTION_ORTHOGONAL
	
	var camera_forward: Vector3 = \
	ota_behavior.camera.global_transform.basis.get_rotation_quaternion() * Vector3.FORWARD
	var camera_to_selected_node_dir: Vector3 = (ota_behavior.selected_node.global_position - \
			ota_behavior.camera.global_position).normalized()
	var projection_scale: float = camera_forward.dot(camera_to_selected_node_dir)
	projection_scale = max(abs(projection_scale), ota_behavior.EPSILON) * sign(projection_scale)
	if is_orthogonal:
	...
	if is_orthogonal:
		gizmo_position = ota_behavior.selected_node.global_position
	var axis_global_position: Vector3 = gizmo_position + side_vec * axis_amplitude
	
	var gizmo_position_2d_in_viewport: Vector2 = global_position
	var axis_local_position_2d: Vector2 = \
	ota_behavior.camera.unproject_position(axis_global_position) - \
			gizmo_position_2d_in_viewport
	var axis_outer_shaft_offset: Vector2 = \
	ota_behavior.camera.unproject_position(axis_global_position + \
			(gizmo_position - axis_global_position) * OUTER_SHAFT_ARC_OFFSET_FACTOR) - \
					gizmo_position_2d_in_viewport

	if is_orthogonal:
		axis_outer_shaft_offset *= ota_behavior.camera.size * gizmo.ORTHOGRAPHIC_SCALE

	axis_local_position_2d *= ota_behavior.relative_scale
	...
	gizmo.viewport_size_factor:
		ota_behavior.dont_detect_collision_axis = axis
	...
	arc_inner_arrow_part.update_drawing(axis_local_position_2d.normalized() \
	...
	ota_behavior.calculate_arrow_head(ota_behavior.TIP_OFFSET_3D, ARROW_STEP_AMOUNT, \
			axis_global_position, gizmo_position_2d_in_viewport, side_vec, front_vec, [axis_color, \
					...
					ota_behavior.ARROW_RADIUS, projection_scale)
	draw_arrow_tip(ARROW_STEP_AMOUNT)
...
func draw_arrow_tip(in_arrow_steps: int):
	# + 2 to fill also the top and the bottom of the arrow head.
	for i in range(0, in_arrow_steps + 2):
		var poly_points: PackedVector2Array = ota_behavior.arrow_tip_sides[i][0]
		var poly_colors: PackedColorArray = ota_behavior.arrow_tip_sides[i][2]
