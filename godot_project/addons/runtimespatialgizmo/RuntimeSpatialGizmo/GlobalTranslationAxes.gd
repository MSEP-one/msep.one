...
func check_axis_collision(axis: Node3D) -> void:
	...
	var axis_collision_position_2d_in_viewport: Vector2 = camera.unproject_position(axis_global_position + \
			(axis_global_position - gizmo.global_position).normalized() * AXIS_COLLISION_OFFSET_FACTOR)
	...
	collision_radius /= projection_scale
...
func _draw() -> void:
	...
	var reference_rotation: Quaternion = Quaternion.IDENTITY
